...
import com.jony.generator.entity.Configuration;
...
public class ConfigUtil {
    ...
    private static void readConfigurationFromFile() {
        try {
            var configUrl = ConfigUtil.class.getClassLoader().getResource("generator.yaml");
            var dbUrl = ConfigUtil.class.getClassLoader().getResource("application.yaml");
            if (null == configUrl || configUrl.getPath().contains("jar")) {
                ...
                var yaml = new Yaml();
                var configConnection = configUrl.openConnection();
                configConnection.setRequestProperty("Accept", "application/json");
                var configInputStream = new InputStreamReader(configConnection.getInputStream());
                var configuration = yaml.loadAs(configInputStream, Configuration.class);

                var dbConnection = dbUrl.openConnection();
                dbConnection.setRequestProperty("Accept", "application/json");
                var dbInputStream = new InputStreamReader(dbConnection.getInputStream());
                var dbMap = (Map) (((Map) ((Map) yaml.load(dbInputStream)).get("spring")).get("datasource"));
                var db = new Configuration.Db((String) dbMap.get("url"), (String) dbMap.get("username"), (String) dbMap.get("password"));
    ...
    private static void checkConfiguration() {
        try {
            ...
            if (null == ConfigUtil.configuration.getName()) {
                var nameConfig = new Configuration.Name();
            ...
            if (null == ConfigUtil.configuration.getDb()) {
                throw new CommonException("Can not find configuration attribute named 'db', please make sure that you have configured that attribute.");
            }
            // 检查path属性
            if (null == ConfigUtil.configuration.getPath()) {
                throw new CommonException("Can not find configuration attribute named 'path', please make sure that you have configured that attribute.");
            }
            // 检查db属性是否配置
            if (StringUtil.isEmpty(ConfigUtil.configuration.getDb().getUrl()) || StringUtil.isEmpty(ConfigUtil.configuration.getDb().getUsername())) {
                throw new CommonException("Please configure the correct db connection parameters, i.e. url, username and password.");
            }
            // 检查顶级包名是否配置
            if (StringUtil.isEmpty(ConfigUtil.configuration.getPackageName())) {
                throw new CommonException("Please configure the correct attribute named 'package-name' or 'packageName'.");
            }
            // 检查是否同时启用了jpa和mybatis-plus模式
            if (ConfigUtil.configuration.isMybatisPlusEnable() && ConfigUtil.configuration.isJpaEnable()) {
                throw new CommonException("Can not enable JPA mode and MyBatis-Plus mode at the same time.");
